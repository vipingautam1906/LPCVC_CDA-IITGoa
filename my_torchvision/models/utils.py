...
########################################################################
############### FIXED HELPERS FOR 3D VIDEO TENSORS #####################
########################################################################
...
class Conv3dDynamicSamePadding(nn.Conv3d):
    ...
    def forward(self, x):
        ...
        ot, oh, ow = math.ceil(it / st), math.ceil(ih / sh), math.ceil(iw / sw)
        
        pad_t = max((ot - 1) * st + (kt - 1) * self.dilation[0] + 1 - it, 0)
        pad_h = max((oh - 1) * sh + (kh - 1) * self.dilation[1] + 1 - ih, 0)
        pad_w = max((ow - 1) * sw + (kw - 1) * self.dilation[2] + 1 - iw, 0)
        
        if pad_t > 0 or pad_h > 0 or pad_w > 0:
            # F.pad for 3D uses 6 values: [W_left, W_right, H_top, H_bottom, T_front, T_back]
...
class Conv3dStaticSamePadding(nn.Conv3d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, image_size=None, **kwargs):
        ...
        # image_size should be [T, H, W]
        it, ih, iw = image_size if isinstance(image_size, list) else [image_size]*3
        kt, kh, kw = self.weight.size()[-3:]
        st, sh, sw = self.stride
        ...
        if pad_t > 0 or pad_h > 0 or pad_w > 0:
            self.static_padding = [pad_w // 2, pad_w - pad_w // 2, 
                                   pad_h // 2, pad_h - pad_h // 2, 
                                   pad_t // 2, pad_t - pad_t // 2]
        else:
...
class BlockDecoder(object):
    ...
    def _decode_block_string(block_string):
        ...
        # --- FIX: Modified stride check to allow non-uniform 3D strides (like 1x2x2) ---
        # Allow 1 digit (standard) or 3 digits (any combination)
        assert ('s' in options and (len(options['s']) == 1 or len(options['s']) == 3))

        if len(options['s']) == 3:
            stride = [int(options['s'][0]), int(options['s'][1]), int(options['s'][2])]
        else:
        ...
        return BlockArgs(
            ...
            stride=stride) # Return the full list
...
def efficientnet3d(width_coefficient=None, depth_coefficient=None, dropout_rate=0.2,
    ...
    """ Creates a efficientnet model with Temporal Stride = 1. """

    # STRATEGY: Change 's222' (Stride 2 in Time/H/W) to 's122' (Stride 1 in Time, 2 in H/W)
    # This keeps our 16 frames alive throughout the network.
    blocks_args = [
        'r1_k3_s122_e1_i32_o16_se0.25', 
